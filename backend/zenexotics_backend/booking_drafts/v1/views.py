...
# class BookingDraftUpdateServiceView(APIView):
#     permission_classes = [IsAuthenticated]
#     renderer_classes = [JSONRenderer]
...
#         try:
#             # Get the new service ID from the request
...
#             # Verify the service exists and belongs to the professional
#             service = get_object_or_404(
#                 Service, 
...
#             # Get occurrences and format them
#             occurrences = []
#             for occurrence in BookingOccurrence.objects.filter(booking=booking):
#                 try:
#                     # Calculate costs for this occurrence
...
#             # If draft exists, load its data
#             current_draft_data = draft.draft_data if draft.draft_data else {}

#             # Create new draft data with updated service details
#             draft_data = create_draft_data(
#                 booking=booking,
#                 request_pets=current_draft_data.get('pets', []),
#                 occurrences=occurrences,
#                 cost_summary=cost_summary
#             )
...
#         except Exception as e:
#             logger.error(f"Error in BookingDraftUpdateServiceView: {e}")
#             return Response(
#                 {"error": "An error occurred while updating the service details"},
#                 status=status.HTTP_500_INTERNAL_SERVER_ERROR
#             )